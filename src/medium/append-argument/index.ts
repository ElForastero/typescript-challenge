...
// Naive but strainforward solution
// type AppendArgument<Fn, T> = Fn extends () => infer R
//   ? (x: T) => R
//   : Fn extends (a: infer A, b: infer B) => infer R
//   ? (a: A, b: B, x: T) => R
//   : never;

type AppendArgument<Fn, T> = Fn extends (...args: infer L) => infer R
  ? (...args: [...L, T]) => R
  : never;
